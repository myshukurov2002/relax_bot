package com.company;

public interface Components {
    String OVERVIEW = """
            🏪ORHIDEYA HOTEL&SPA🏪
            
            Assalomu alaykum! 😊
            Hurmatli mijoz, bizni tanlaganingizdan xursandmiz! Sizga avtomatik bot javob bermoqda 👩🏻‍💻
            📍 Bizning manzil va narxlar bilan tanishishingiz mumkin.
            Agar ma'qul kelsa, lokatsiyamiz orqali bemalol tashrif buyurishingiz mumkin.
            Biz 24/7 sizning xizmatdamiz! 😊""";

    String START = "/start";
    String LOCATION = "📍 Manzil Olish";
    String PRICES = "\uD83D\uDCB8 Narxlar";
    String PRICE_1 = "\uD83D\uDCB0 450 mingdan 3 million so'mgacha";
    String PRICE_2 = "\uD83D\uDCB0 500 mingdan 1 million 300 ming so'mgacha";
    String PRICE_3 = "\uD83E\uDDD1\uD83C\uDFFB\u200D\uD83D\uDCBC Xizmatlar:\n\n\uD83D\uDCB0 450 mingdan 3 million so'mgacha\n\uD83D\uDCB0 500 mingdan 1 million 300 ming so'mgacha";

    Double longitude = 69.257473;
    Double latitude = 41.287429;
    String ERROR = "Xato Buyruq! Menyuga qaytish uchun /start bosing! \uD83D\uDE42\n/start\n/start";

    String PHONE_1 = "\n\n☎\uFE0F Raqamlar:\n \uD83C\uDFEA Hotel: +998992049393\n \uD83D\uDC86\uD83C\uDFFB Massage 24/7: +998992049393";
    String PHONE_2 = "\n\n☎\uFE0F Telefon Raqam: +998981015702";

    String PHOTO_FIELD_1 = "AgACAgIAAxkBAANEZyx2PaRzIX6AMnhX5m8ofPeoW50AAjjgMRtjemhJyJWmKP9MCDcBAAMCAAN5AAM2BA";
    String PHOTO_FIELD_2 = "AgACAgIAAxkBAAMzZyypCQSDviFKdiPWmOI-vOi5DuUAAqruMRv6M2FJulRa0QNs9XYBAAMCAAN5AAM2BA";
    String WILL_CONTACT = "Hurmatli Mijoz! Reklama kanallardan bizni tanlaganingizdan mamnunmiz\uD83D\uDE0A\n" +
            "Sizga Tez orada Kanal Admini javob beradi kutib qoling…";
}
